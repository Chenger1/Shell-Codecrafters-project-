...
impl CommandLineHelper {
    ...
    fn filename_completion(
    ...
    ) -> Option<rustyline::Result<(usize, Vec<<CommandLineHelper as Completer>::Candidate>)>> {
        ...
        if splitted_line.len() > 1 {
            ...
            if let Ok((start, candidates)) = result {
                let updated_candidates: Vec<RustyPair> = candidates
                    .into_iter()
                    .map(|mut candidate| {
                ...
                let candidates_strings: Vec<String> = updated_candidates
                    .clone()
                    .into_iter()
                ...
                if prefix.len() > splitted_line[1].to_string().len() {
                    return Some(Ok((
                        splitted_line[0].to_string().len() + 1,
                        vec![RustyPair {
...
impl Completer for CommandLineHelper {
    ...
    fn complete(
        ...
        // Path completion
            ...
            if let Some(res) = self.multiple_completions(matched, &line.to_string(), None){
                return res;
            }
            return Ok((0, result));
        }
